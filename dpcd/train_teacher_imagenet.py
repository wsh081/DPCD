...
parser.add_argument('--pretrained-backbone', default='./pretrained_models/wrn_40_2_aux.pth', type=str,
...
def test(epoch, criterion_cls):
    ...
    with torch.no_grad():
        ...
        class_acc1 = [round((ss_top5_num[i] / (total)).item(), 4) for i in range(num_auxiliary_branches)] + [
            round((top1_num / (total)).item(), 4)]
        class_acc5 = [round((ss_top5_num[i] / (total)).item(), 4) for i in range(num_auxiliary_branches)] + [
            round((top5_num / (total)).item(), 4)]
        with open(log_txt, 'a+') as f:
