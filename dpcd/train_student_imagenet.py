...
def main_worker(gpu, ngpus_per_node, args):
    ...
    criterion_w = InfoMaxLoss56(args.kd_T)
    ...
    criterion_crd = InfoMaxLoss1(args.kd_T)
    ...
    def train(epoch, criterion_list, optimizer):
        ...
        criterion_a = criterion_list[3]
        criterion_crd = criterion_list[4]
        ...
        for batch_idx, (input, target) in enumerate(trainloader):
            ...
            for i in range(1, len(t_ss_logits)):
                for j in range(i):
                    loss_div += criterion_div(ss_logits[j], t_ss_logits[i].detach())
            for i in range(len(ss_logits)):
                loss_w += criterion_w(ss_logits[i], t_logits.detach())
            #
            #

            #
            for i in range(len(t_ss_logits) - 1):
                for j in range(i, len(ss_logits)):
                    loss_a += criterion_a(ss_logits[j], t_ss_logits[i].detach())

            #
            for i in range(len(ss_logits) - 1):
                for j in range(i, len(ss_logits)):
                    loss_crd += criterion_crd(ss_logits[j], ss_logits[i].detach())
            for i in range(len(ss_logits)):
                loss_crd += criterion_crd(logits, ss_logits[i].detach())

            loss = loss_cls + loss_div + loss_w + loss_a + loss_crd
            #
        ...
        with open(args.log_txt, 'a+') as f:
            f.write('Epoch:{}\t lr:{:.5f}\t duration:{:.3f}'
                    ...
                    .format(epoch, lr, time.time() - start_time,
                            train_loss, train_loss_cls, train_loss_div, train_loss_w, train_loss_a, train_loss_crd,
    ...
    def test(epoch, criterion_cls, net):
        ...
        with torch.no_grad():
            ...
            ss_acc1 = [round((ss_top1_num[i] / (total)).item(), 4) for i in range(len(ss_logits))] + [
                round((top10_num / (total)).item(), 4)]
            max_index = max(range(len(ss_acc1) - 1), key=ss_acc1.__getitem__)
            ss_acc1[max_index], ss_acc1[-1] = ss_acc1[-1], ss_acc1[max_index]
            class_acc1 = ss_acc1 + [round((top1_num / (total)).item(), 4)]
...
class DistillKL(nn.Module):
    ...
    def forward(self, y_s, y_t):
        ...
        return loss
class InfoMaxLoss56(nn.Module):
    def __init__(self, T, alpha=1.0):
        super(InfoMaxLoss56, self).__init__()
        self.alpha = alpha
        self.T = T

    def forward(self, student_logits, teacher_logits):
        # 计算教师和学生的概率分布，使用温度 T 平滑
        teacher_probs = F.softmax(teacher_logits/self.T , dim=1)
        student_probs = F.softmax(student_logits /self.T, dim=1)

        # 近似的联合概率分布计算（取教师和学生对数分布的加权平均）
        joint_log_probs = (teacher_probs +  student_probs )/2

        # 使用 softmax 归一化来确保 joint_probs 是有效的概率分布
        joint_probs = F.log_softmax(joint_log_probs, dim=1)

        # 计算 InfoMax 损失，基于联合概率和教师概率的 KL 散度
        infomax_loss = F.kl_div(joint_probs, teacher_probs, reduction='batchmean') * (self.T ** 2)


        # 最终损失
        loss = self.alpha * infomax_loss
        return loss

class InfoMaxLoss1(nn.Module):
    def __init__(self, T,alpha=1.0):
        super(InfoMaxLoss1, self).__init__()
        self.alpha = alpha
        self.T = T
    def forward(self, student_logits,teacher_logits):
        teacher_probs = F.softmax(teacher_logits/self.T, dim=1)
        student_probs = F.softmax(student_logits/self.T, dim=1)
        joint_probs = (teacher_probs * student_probs)
        joint_probs /= joint_probs.sum(dim=1, keepdim=True)
        infomax_loss = F.kl_div(joint_probs,teacher_probs, reduction='batchmean') * (self.T ** 2)
